...
def get_hand_type(cards) :
    ...
    for c in cards :
        if c == 'J' :
            wildcards+= 1
        elif c in card_counts :
            card_counts[c] += 1
        else:
            card_counts[c] = 1
    sorted_cards = sorted(card_counts.values(), reverse=True)
    print(sorted_cards, wildcards)
    if wildcards == 5 : 
        return 6
    elif sorted_cards[0] + wildcards == 5:
        return 6
    elif sorted_cards[0] + wildcards == 4:
        return 5
    elif sorted_cards[0] + wildcards == 3  and sorted_cards[1] == 2 :
        return 4
    elif sorted_cards[0] + wildcards == 3 :
        return 3
    elif sorted_cards[0] + wildcards == 2 and sorted_cards[1] == 2 :
        return 2
    elif sorted_cards[0] + wildcards == 2 and sorted_cards[1] == 1:
        return 1
    return 0
